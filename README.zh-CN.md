...
如果你在开发时使用 Codex App 的插件管理界面，请导入 clone 下来的仓库根目录。
...
| `DEEPSEEK_FORGE_HOME` | 否 | 已安装插件中的 `skills/deepseek-forge` | deepseek-forge 的 skill 根目录，也就是包含 `SKILL.md` 和 `references/prompt_templates.md` 的目录。 |
| `DEEPSEEK_FORGE_SESSION_ID` | 否 | 无 | 可选的单次对话命名空间，用于默认产物路径。 |
| `DEEPSEEK_FORGE_ARTIFACT_DIR` | 否 | `/tmp/deepseek-forge-{pid}/` 或 `/tmp/deepseek-forge-{session}-{pid}/` | 运行时产物目录，包括 patch、日志和上下文文件。设置为 `.deepseek-forge` 可把产物保留在目标仓库中。 |
| `DEEPSEEK_FORGE_LOCK_PATH` | 否 | `.git/deepseek-forge.lock` | apply/check 等操作使用的仓库级锁目录。 |
| `DEEPSEEK_FORGE_DISABLE_REPO_LOCK` | 否 | 未设置 | 只有明确确认安全时，才设为 `1` 绕过仓库锁。 |
...
运行时文件写入 `DEEPSEEK_FORGE_ARTIFACT_DIR`，默认 `/tmp/deepseek-forge-{pid}/`；如果设置了 `DEEPSEEK_FORGE_SESSION_ID`，则默认路径为 `/tmp/deepseek-forge-{session}-{pid}/`。设置 `DEEPSEEK_FORGE_ARTIFACT_DIR=.deepseek-forge` 可将产物保留在目标仓库中，也可以设置成任意自定义路径。

## 并发使用

多个 Codex 对话可以同时使用已安装的插件。如果它们操作的是不同仓库或不同 git worktree，通常不需要额外配置。

如果多个对话操作同一个 repository worktree，deepseek-forge 会用仓库级锁保护写操作。默认锁目录是 `.git/deepseek-forge.lock`；非 Git 目录下会退回到 `.deepseek-forge/deepseek-forge.lock`。`apply_patch_safe.py --apply` 和 `run_checks.sh` 会获取这个锁，避免多个会话同时 apply patch 或覆盖 check log。

为了让不同对话的产物路径更清晰，可以设置单次会话 id：
...
如果是在 deepseek-forge 仓库内部开发或调试，请使用插件包内的 skill 脚本路径：
...
如果只想做 smoke test、不影响真实 Codex 配置：
